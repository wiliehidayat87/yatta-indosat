...
class model_user extends model_base {
	...
	*/
           $sql = sprintf("SELECT DISTINCT msisdn FROM subscription_insuf", mysql_escape_string($broadcast_data->service), mysql_escape_string($broadcast_data->adn), mysql_escape_string($broadcast_data->operator));
	//}
        $data = $this->databaseObj->fetch($sql);
        if (count($data) > 0) {
            return $data;
	...
	public function checkMsisdnSubject($msisdn) {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "Start : " . $msisdn));

        $sql = sprintf("SELECT subject, notif FROM msisdn_subject WHERE msisdn = %s LIMIT 1", mysql_real_escape_string($msisdn));
        $data = $this->databaseObj->fetch($sql);
        if (count($data) > 0) {
            return $data;
        } else {
	...
	public function updateMsisdnSubject($msisdn, $subject = "NORMAL") {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "Start msisdn : " . $msisdn));

        $sql = sprintf("UPDATE msisdn_subject SET subject = '%s'", mysql_real_escape_string($subject));
        $sql .= sprintf(" WHERE msisdn = '%s';", mysql_real_escape_string($msisdn));
        $this->databaseObj->query($sql);
        return true;
    }
	
	public function insertMsisdnSubject($msisdn, $subject = "NORMAL") {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "Start msisdn : " . $msisdn));

        $sql = sprintf("INSERT INTO msisdn_subject (msisdn,subject) VALUE ('%s','%s')", mysql_real_escape_string($msisdn), mysql_real_escape_string($subject));
        $this->databaseObj->query($sql);
        return true;
    }
}
