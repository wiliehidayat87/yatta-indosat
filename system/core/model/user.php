...
class model_user extends model_base {
    ...
    public function get($user_data) {
        ...
        $sql = "SELECT * FROM {$dbname}.subscription WHERE ";
    ...
    public function getException($user_data) {
        ...
        $sql = "SELECT * FROM {$dbname}.subscription WHERE `active` <> " . mysql_real_escape_string($active);
	...
	if($broadcast_data->service == 'game'){
	   if(date('G')<=12){
	   	$sql = sprintf("SELECT msisdn FROM subscription WHERE service = '%s' AND adn = '%s' AND operator = '%s' AND active = '1' ORDER BY id ASC LIMIT 200000", mysql_escape_string($broadcast_data->service), mysql_escape_string($broadcast_data->adn), mysql_escape_string($broadcast_data->operator));
	   }else{
		$sql = sprintf("SELECT msisdn FROM subscription WHERE service = '%s' AND adn = '%s' AND operator = '%s' AND active = '1' ORDER BY id ASC LIMIT 200000, 400000", mysql_escape_string($broadcast_data->service), mysql_escape_string($broadcast_data->adn), mysql_escape_string($broadcast_data->operator));
	   }
	}else{
	*/
           $sql = sprintf("SELECT DISTINCT msisdn FROM subscription WHERE service = '%s' AND adn = '%s' AND operator = '%s' AND active = '1' ORDER BY partner DESC", mysql_escape_string($broadcast_data->service), mysql_escape_string($broadcast_data->adn), mysql_escape_string($broadcast_data->operator));
	//}
        $data = $this->databaseObj->fetch($sql);
        if (count($data) > 0) {
            return $data;
    ...
    public function checkBlacklist($msisdn) {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "is msisdn " . $msisdn . " blacklist ?"));
		
        $sql = "SELECT * FROM blacklist WHERE msisdn = " . mysql_real_escape_string($msisdn);
        $sql .= " ORDER BY msisdn DESC LIMIT 1";
	...
	public function checkMsisdnSubject($data) {
        $log = manager_logging::getInstance();
        ...
        $sql = sprintf("SELECT subject, notif FROM {$dbname}.msisdn_subject WHERE msisdn = %d AND operator = '%s' AND service = '%s' LIMIT 1", mysql_real_escape_string($data['msisdn']), mysql_real_escape_string($data['operator']), mysql_real_escape_string($data['service']));
        $data = $this->databaseObj->fetch($sql);
        if (count($data) > 0) {
            return $data;
        } else {
	...
	public function updateMsisdnSubject($data) {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "Start msisdn : " . serialize($data)));

		$config_db = loader_config::getInstance()->getConfig('database');
		$dbname = $config_db->profile['connDatabase1']['database'];
		
        $sql = sprintf("UPDATE {$dbname}.msisdn_subject SET `subject` = '%s', `subject2` = '%s' WHERE msisdn = %d AND operator = '%s' AND service = '%s'", mysql_real_escape_string($data['subject']), mysql_real_escape_string($data['subject2']), mysql_real_escape_string($data['msisdn']), mysql_real_escape_string($data['operator']), mysql_real_escape_string($data['service']));
        $this->databaseObj->query($sql);
        return true;
    }
	
	public function insertMsisdnSubject($data) {
        $log = manager_logging::getInstance();
        $log->write(array('level' => 'debug', 'message' => "Start msisdn : " . serialize($data)));
        ...
        $sql = sprintf("INSERT INTO {$dbname}.msisdn_subject (msisdn,operator,service,`subject`,`subject2`) VALUE ('%s','%s','%s','%s','%s')", mysql_real_escape_string($data['msisdn']), mysql_real_escape_string($data['operator']), mysql_real_escape_string($data['service']), mysql_real_escape_string($data['subject']), mysql_real_escape_string($data['subject2']));
        $this->databaseObj->query($sql);
        return true;
    }
	...
	public function getMembership($data) {
        ...
        $sql = sprintf("SELECT * FROM xmp.membership WHERE service = '%s' AND operator = '%s' AND msisdn = '%s' LIMIT 1", mysql_escape_string($data->service), mysql_escape_string($data->operator), mysql_escape_string($data->msisdn));
        $data = $this->databaseObj->fetch($sql);
        if (count($data) > 0) {
            return $data;
        } else {
