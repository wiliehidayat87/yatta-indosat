...
class indosat_model_mtdelay extends model_base {
    ...
    public function add(mt_delay_data $mt_delay) {
        ...
        $sql = sprintf("
                    INSERT INTO mt_delay (
                            ...
                            '%s', '%s', '%s', '%s', NOW(), DATE_ADD(NOW(),INTERVAL %d SECOND), 2 
                    )", mysql_real_escape_string($mt_delay->obj), mysql_real_escape_string($mt_delay->service), mysql_real_escape_string($mt_delay->adn), mysql_real_escape_string($mt_delay->msisdn), mysql_real_escape_string($delay_config->mtDelay));
    ...
    public function get($user) {
        ...
        $sql = sprintf("SELECT * FROM mt_delay WHERE msisdn = '%s' AND status = '2' ORDER BY id DESC LIMIT 1;", mysql_real_escape_string($user));
    ...
    public function getExpired() {
        ...
        $delay_config = loader_config::getInstance()->getConfig('delay');
        $sql = sprintf("SELECT * FROM mt_delay WHERE date_expired < NOW() AND status = '2' LIMIT %d;", mysql_real_escape_string($delay_config->mtDelay));
