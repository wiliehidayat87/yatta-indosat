...
$(document).ready(function() {
    ...
    $("#serviceName").autocomplete({
        ...
        source: function(req, add){
            $.ajax({
                url: base_url + "traffic/mo_traffic/ajaxGetServiceList",
                dataType: 'json',
                type: 'POST',
                data: req,
    ...
    $(".pagination ul li a").live("click", function() {
        getMOTrafficList($(this).attr("href"),'');
    ...
    $("#mo-traffic-form").submit(function() {
        ...
        if ($("#search").val() == "Search")
            url = base_url + "traffic/mo_traffic/ajaxGetMOTrafficList";
        
        gbl_data = searchData;
        
        getMOTrafficList(url, gbl_data);        
...
function getMOTrafficList(url, dataSearch) {
    if (url == '')
        url = base_url + "traffic/mo_traffic/ajaxGetMOTrafficList";
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.status == true) {
                ...
                if (data.status_dateRange == false) {
                    $("#dateFrom").addClass("error-field");
                    $("#dateTo").addClass("error-field");
                    $.achtung({
                        timeout: 5, // Seconds
                        className: 'achtungFail',
                        icon: 'ui-icon-check',
                        message: 'Date range must fill both'
                    });
                }
                else {
                    $("#dateFrom").removeClass("error-field");
                ...
                if (data.status_serviceName == false) {
                    $("#serviceName").addClass("error-field");
                    $.achtung({
                        timeout: 5, // Seconds
                        className: 'achtungFail',
                        icon: 'ui-icon-check',
                        message: 'Services is not available'
                    });
                }
                else {
                    $("#serviceName").removeClass("error-field");
