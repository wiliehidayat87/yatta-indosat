	    ...
	    {if $subject != false}
		    {foreach from=$subject item=row name=o}
	          <tr {if ($smarty.foreach.o.iteration%2) == 0}class="alt"{/if}>
	          	  {if $row.name == 'total'}
	          	      <td style="text-align: center;"><b>{$row.name|upper}</b></td>
			  	  	  <td style="display: none;"></td>
	          	  {else}
	          	      <td title="{$row.info}" style="text-align: left;cursor:pointer;" class="pointerBD"><span class="collapsed">{$row.name}</span></td>
			  	  	  <td style="display: none;"></td>
	          	  {/if}
			  </tr>
			{/foreach}
