    ...
    <table class="datagrid2 dg2-left" width="200px">
        ...
        {foreach from=$mostLeftColumn item=row name=i}
            <tr id="{$row.id}-{$row.name}">
                {if $row.name == 'NEW' || $row.name == 'UNREG' || $row.name == 'UNREG RATE' || $row.name == 'MT' || $row.name == 'MT BILLABLE' || $row.name == 'GROSS' || $row.name == 'GROSS RATE'}
                    <td class="mostLeftColumn" style="padding:2px 2px 0 2px;text-align:left;"><span class="{if $smarty.foreach.i.iteration > 5}{if $row.name != 'GROSS' && $row.name != 'NEW' && $row.name != 'UNREG' && $row.name != 'UNREG RATE' && $row.name != 'MT' && $row.name != 'MT BILLABLE' && $row.name != 'GROSS RATE'}collapsed {/if}{/if}getCharging {if ($row.name == 'MO' || $row.name == 'DELIVERED') && ($smarty.foreach.i.iteration > 5)}pointer{/if}" id="{$row.id}" title="{$row.name|upper}">&nbsp;&nbsp;&nbsp;{$row.name|upper} {if ($row.name == 'DELIVERED') && ($smarty.foreach.i.iteration > 5)}(Toggle Charging){/if}</span></td>
                {else}
