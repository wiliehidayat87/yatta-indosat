...
class default_service_module_waplink implements service_module_interface {
    ...
    public function run(mo_data $mo, service_reply_data $reply) {
        ...
        $content->initialChargeType = 'free';

        $content->wapName = $replyAttributes ['wapdownload_name'];

        #save New Token | return Obj Content_data
        $saveNewToken = content_manager::getInstance()->getContent($content);
    ...
    private function getMsg(mo_data $mo, service_reply_data $reply, array $replyAttributes) {
        ...
        if ($reply->sequence < 1) {
            $mt->subject = strtoupper('MT;PULL;' . $mo->channel . ';WAPLINK');
            //$mt->type = 'mtpush';
            $mt->inReply = $mo->inReply;
        } else {
            $mt->subject = strtoupper('MT;PUSH;' . $mo->channel . ';WAPLINK');
            //$mt->type = 'mtpush';
            $mt->inReply = null;
        }
        $mt->mo = $mo;
