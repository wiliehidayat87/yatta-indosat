<?php class indosat_broadcast_base extends default_broadcast_base {
	/**
	...
	public function push(broadcast_data $broadcast_data) {
		...
		if ($users !== false) {
			...
			$split_service = array("dangdut", "game");
			$split_day = array(1,3,5,7);
			...
			foreach ( $users as $users_data ) {
				...
				/*
				if(in_array($broadcast_data->service,$split_service)){
					if(in_array(date("N"),$split_day)){
					  if(intval($users_data ['msisdn'])%2>0)){
				...
				*/
				...
				// Disable Ganjil Genap request on 02082016
				/*
				if((($broadcast_data->service == 'dangdut')||($broadcast_data->service == 'game'))&&(intval($users_data ['msisdn'])%2>0)){
					$pushbuffer_data->stat = "PENDING";				
				}
				*/
	...
	public function sendtoQueue() {
		...
		$limit = isset($params['n'])?$params['n']:1000;
		$log = manager_logging::getInstance ();
		$log->write ( array ('level' => 'debug', 'message' => "Start limit ".$limit ) );
		...
		foreach ( $pushProjects as $pushProject ) {
			...
			//if($pushProject['service'] == 'game'){
			//	$pid_services[$pushProject['service']] = '1114';
			//}else{
